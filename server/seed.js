...
import { User, Account, Transaction, Beneficiary, Card, Notification, Biller } from './models/index.js'
...
await Transaction.deleteMany({ visibility: 'shared', owner: { $exists: false } })
await Transaction.bulkWrite(TRANSACTIONS.map((source) => ({ updateOne: { filter: { reference: source.reference }, update: { $setOnInsert: { owner: undefined, visibility: 'shared', account: primary._id, type: source.type, direction: source.type, category: source.category, description: source.description, counterparty: source.counterparty, amount: source.amount, fee: source.fee, status: source.status, date: source.date, reference: source.reference, narration: source.narration, meta: source.meta } }, upsert: true } })))
